...
生产环境必须设置 `DATABASE_URL`、`CORS_ORIGINS`、`NODE_ENV=production`、`PORT` 和 `HOST`。应用启动前执行迁移，应用进程不自动改表：
...
`release:build` 会先构建 workspace，再准备包含编译 workspace 包和 `v1.2.0` 正式内容的 API 运行目录；`release:smoke` 会用纯 Node 启动该编译入口并校验 `/ready` 的内容版本与校验和；`start:v11` 是正式 v1.2 API 的非 watch 启动入口，`dev:v11` 只用于本地开发。启动前仍必须完成迁移，并设置生产数据库、密钥、HTTPS 反向代理和明确的 `CORS_ORIGINS`。
...
备份脚本默认随当前发布候选归档 `content/compiled/v1.2.0.json`；若备份历史版本，需同时设置 `CONTENT_VERSION` 和对应的 `CONTENT_FILE`，脚本会在执行 `pg_dump` 前拒绝版本不匹配的内容包。manifest 中的 `contentChecksum` 必须与该文件的 SHA-256 一致。
...
恢复脚本默认用 `content/compiled/v1.2.0.json` 校验备份 manifest；恢复历史版本时必须同时设置 `CONTENT_VERSION` 和 `CONTENT_FILE`。在执行 `pg_restore` 前，版本或 SHA-256 任一不匹配都会停止，不会写入临时数据库。
